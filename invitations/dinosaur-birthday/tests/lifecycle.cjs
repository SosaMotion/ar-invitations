...
async function test(mode){
 ...
 class El{
  constructor(tag){this.tag=tag;this.events={};this.attrs={};this.hidden=true;this.classList={add(){},remove(){}};this.components={};this.object3D={position:{z:0}};}
  addEventListener(name,cb){(this.events[name]??=[]).push(cb)}
  emit(name,detail={}){for(const cb of this.events[name]||[])cb({detail})}
  setAttribute(key,value){this.attrs[key]=value;if(key==='birthday-animation') { component.instance={el:this}; component.value.init.call(component.instance); }}
 ...
 const ctx={window,document,navigator:{mediaDevices:{getUserMedia(){}}},console,Uint8Array,AbortSignal,Blob,URL,setTimeout:()=>1,clearTimeout(){},AFRAME:{registerComponent(name,c){component.value=c},THREE:{AnimationMixer:class{clipAction(){return action}update(){}},LoopRepeat:1}},fetch:async url=>({ok:mode!=='missing',json:async()=>({autoFit:false,animationClip:mode==='bad-clip'?'absent':''}),headers:{get(){return 'application/octet-stream'}},arrayBuffer:async()=>new TextEncoder().encode(url.includes('.glb')?'glTFdata':'target').buffer})};
 ...
 if(['insecure','missing','bad-clip'].includes(mode)){assert.equal(nodes.error.hidden,false);assert.equal(cameraStarts,0);return;}
 ...
 anchor.emit('targetFound');assert.equal(nodes.model.attrs.visible,true);if(mode!=="no-clip"){assert.equal(action.paused,false);assert.equal(resets,1);}else{component.value.tick.call(component.instance,100,100);assert(nodes.model.object3D.position.z>0);}
 ...
 anchor.emit('targetFound');nodes.replay.emit('click');if(mode!=="no-clip")assert.equal(resets,3);
 document.hidden=true;document.emit('visibilitychange');assert.equal(nodes.error.hidden,false);assert(stopped>0);
}
(async()=>{for(const mode of ['insecure','missing','bad-clip','no-clip','tracking']){await test(mode);console.log('PASS',mode)}})().catch(e=>{console.error(e);process.exit(1)});
